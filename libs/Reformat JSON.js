...
<javascriptresource>
<name>Reformat JSON...</name>
<about>"Reformat JSON" v1.2

Reformat JSON text.

Utility script using the "JSON Action Manager" scripting library.
© 2013-2014 Michel MARIANI.
</about>
<menu>automate</menu>
<category>JSON Action Manager JSON Utility</category>
</javascriptresource>

*/

//------------------------------------------------------------------------------
// File: Reformat JSON.js
// Version: 1.2
// Release Date: 2014-02-12
// Copyright: © 2013-2014 Michel MARIANI <http://www.tonton-pixel.com/blog/>
...
//  1.1:
//  - Added user option: strict validation.
//  1.0:
//  - Initial release.
//------------------------------------------------------------------------------

// jamEngine.jsxinc v4.0 (minified)
...
var signature = "json-action-manager-reformat-json-options";

var defaultOptions =
{
	defaultFile: "~/Desktop/text.json",
...
function displayDialog ()
{
	...
	var dialog = new Window ('dialog', "Reformat JSON", undefined, { resizeable: true });
	...
	dialog.onShow = function ()
	{
		...
		strictValidationCheck.value = customOptions.strictValidation;
		expandTabsCheck.value = customOptions.expandTabs;
		addTrailingBreakCheck.value = customOptions.addTrailingBreak;
		jsonArea.active = true;
		validText.text = "";
	};
	var jsonGroup = dialog.add ('group');
	jsonGroup.orientation = "column";
	jsonGroup.alignment = [ "fill", "fill" ];
	var jsonArea = jsonGroup.add ('edittext', undefined, "", { multiline: true });
	jsonArea.alignment = [ "fill", "fill" ];
	jsonArea.minimumSize = [ -1, 120 ];
	if (monospacedFont)
	{
		jsonArea.graphics.font = monospacedFont;
	}
	jsonArea.onChanging = function ()
	{
		validText.text = "";
	}
	var actionButtonsGroup = jsonGroup.add ('group');
	actionButtonsGroup.alignment = [ "center", "bottom" ];
	actionButtonsGroup.orientation = "row";
	actionButtonsGroup.alignChildren = "bottom";
	var clearButton = actionButtonsGroup.add ('button', undefined, 'Clear');
	...
	var fileIn;
	var fileOut;
	var loadButton = actionButtonsGroup.add ('button', undefined, 'Load...');
	loadButton.onClick = function ()
	{
		...
		var jsonFilter = (File.fs === "Macintosh") ? function (f) { return (f instanceof Folder) || isJsonFile (f) } : "JSON Files:*.json;,All Files:*";
		fileIn = defaultFile.openDlg ("Load JSON from file:", jsonFilter);
		if (fileIn)
		{
			jsonArea.text = jamUtils.readTextFile (fileIn);
			validText.text = "";
			defaultFile = fileIn;
		}
		jsonArea.active = true;
	...
	{
		fileOut = defaultFile.saveDlg ("Save JSON to file:");
		if (fileOut)
		{
			jamUtils.writeTextFile (fileOut, jsonArea.text);
			if (openWithEditor)
			{
				fileOut.execute ();
			}
	...
	var reformatGroup = jsonGroup.add ('group');
	reformatGroup.orientation = "column";
	var optionsGroup = reformatGroup.add ('group');
	optionsGroup.orientation = "row";
	var strictValidationCheck = optionsGroup.add ('checkbox', undefined, 'Strict Validation');
	var expandTabsCheck = optionsGroup.add ('checkbox', undefined, 'Expand Tabs');
	var addTrailingBreakCheck = optionsGroup.add ('checkbox', undefined, 'Add Trailing Break');
	var reformatButton = reformatGroup.add ('button', undefined, 'Reformat');
	reformatButton.alignment = [ "center", "top" ];
	reformatButton.onClick = function ()
	{
		try
		{
			jsonArea.text = jamJSON.stringify (jamJSON.parse (jsonArea.text, strictValidationCheck.value), expandTabsCheck.value ? 4 : '\t')
							+
							(addTrailingBreakCheck.value ? "\n" : "");
			validText.text = "Valid JSON"
		}
		catch (e)
		{
			if (beepIfInvalid)
			...
			validText.text = "*Invalid JSON*"
		}
	};
	var validText = reformatGroup.add ('statictext', undefined, '0123456789012345678901234567890123456789');
	validText.justify = "center";
	...
	okButton.onClick = function ()
	{
		...
		customOptions.strictValidation = strictValidationCheck.value;
		customOptions.expandTabs = expandTabsCheck.value;
		customOptions.addTrailingBreak = addTrailingBreakCheck.value;
